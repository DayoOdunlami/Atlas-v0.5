"""
Composition policy — free-form default; reference recipes only when judged higher value.

Recipes win when the skeleton already carries a validated, high-fidelity instrument
that free HTML would struggle to match (two-tier field, force graph, gap matrix,
...
def recommend_worthy_recipe(
    ...
    """Return a reference recipe only when recipe lock is enabled and it beats free compose."""
    if not RECIPE_LOCK_ENABLED:
        return None
    ...
    if wide.outcome == "connect" and recipe == "NetworkMap" and wide.graph:
        ...
        if edge_count >= 2 or wide.graph.ladder_rung == "force-graph":
            return RecipeRecommendation(
                recipe,
    ...
    if wide.outcome == "orient" and recipe == "IncommensurableMagnitudes":
        ...
        if not (data.get("upper") and data.get("lower")):
            return None
        # Only lock the golden two-tier rail orient when explicitly enabled
        if not RECIPE_LOCK_ENABLED:
            return None
