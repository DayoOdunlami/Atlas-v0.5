...
function UserMessage() {
  return (
    <MessagePrimitive.Root className="flex justify-end mb-4 group">
...
function AssistantMessage() {
  ...
  return (
    <MessagePrimitive.Root className="flex gap-2.5 mb-4 group">
      {/* Avatar */}
      ...
      {/* Bubble */}
      <div className="flex-1 min-w-0 max-w-[72ch] rounded-lg px-3.5 py-2.5 text-sm bg-background border border-border leading-relaxed">
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4 text-center gap-3">
      ...
      <div>
        <p className="text-sm font-semibold text-foreground">Atlas Copilot</p>
        <p className="text-[13px] text-muted-foreground mt-1 max-w-[260px] leading-relaxed">
          Ask anything — explore the CPC corpus, add blocks to the artifact,
          or analyse this match. Changes apply live with undo.
        </p>
      </div>
      {/* Mode pills */}
      <div className="flex flex-wrap justify-center gap-1.5">
        {(["Explain", "Search", "Explore", "Economic"] as const).map((m) => (
          <span
            key={m}
            className="text-[11px] px-2 py-0.5 rounded border border-border text-muted-foreground font-medium"
          >
            {m}
          </span>
        ))}
...
function SuggestionChip({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleClick}
      className="text-[12px] rounded-full border border-border px-3 py-1.5 bg-background text-muted-foreground hover:text-foreground hover:border-foreground/30 transition-colors whitespace-nowrap"
    >
      {text}
    </button>
  );
  ...
  return (
    <ThreadPrimitive.Root className="flex flex-col h-full border-r border-border bg-muted/20">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border bg-background shrink-0">
        ...
        <ThreadPrimitive.If running>
          <span
            className="ml-1 inline-flex items-center gap-1.5 text-[11px] rounded px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-800 max-w-[260px] font-medium"
        ...
        <ThreadPrimitive.If running={false}>
          {lastRoute ? (
            ...
            <span
              className={cn(
                "ml-1 inline-flex items-center gap-1.5 text-[11px] rounded px-2 py-0.5 border font-medium",
                session.matchId
      ...
      <ThreadPrimitive.Viewport className="flex-1 overflow-y-auto min-h-0 overscroll-contain px-4 py-3">
