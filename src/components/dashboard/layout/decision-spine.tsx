...
import { DecisionSpine, ConfidenceTier } from "@/lib/types";
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 text-xs">
        <div>
          <p className="font-medium text-muted-foreground mb-0.5">Key assumption</p>
          <p className="text-foreground/80">{spine.key_assumption}</p>
        </div>
        <div>
          <p className="font-medium text-muted-foreground mb-0.5">Next action</p>
          <p className="text-foreground/80">{spine.next_action}</p>
        </div>
        {spine.strongest_objection && (
          <div>
            <p className="font-medium text-muted-foreground mb-0.5">Strongest objection</p>
            <p className="text-foreground/80">{spine.strongest_objection}</p>
          </div>
        )}
        {spine.would_change_if && (
          <div>
            <p className="font-medium text-muted-foreground mb-0.5">Would change if</p>
