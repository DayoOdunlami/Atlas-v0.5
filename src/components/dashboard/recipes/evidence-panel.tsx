...
import { ArtifactBlock, CorpusCitation, SourceType } from "@/lib/types";
import { cn } from "@/lib/utils";

const SOURCE_BADGE: Record<SourceType, { label: string; style: string }> = {
  project:        { label: "R&D Project",  style: "bg-indigo-50 text-indigo-700 border-indigo-200" },
...
function SourceBadge({ type }: { type?: SourceType }) {
  ...
  return (
    <span className={cn("text-xs px-1.5 py-0.5 rounded border font-medium", badge.style)}>
      {badge.label}
    </span>
  );
}
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-3 space-y-2 hover:bg-muted/20 transition-colors">
      <div className="flex items-start justify-between gap-2">
        <p className="text-sm font-medium leading-snug flex-1 line-clamp-2">{c.title}</p>
        <SourceBadge type={c.source_type} />
      </div>
      <div className="flex items-center justify-between">
        {secondary && (
          <span className="text-xs text-muted-foreground truncate max-w-[60%]">{secondary}</span>
        )}
